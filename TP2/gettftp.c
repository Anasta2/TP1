...
int main(int argc, char *argv[]) {
    ...
    // Add Opcode (1 for RRQ)
    buffer[rrq_len++] = 0x00;
    buffer[rrq_len++] = 0x01;

    // Add Filename (null-terminated)
    strcpy(&buffer[rrq_len], filename);
    rrq_len += strlen(filename) + 1;  // +1 for null character

    // Add Transfer Mode (null-terminated)
    strcpy(&buffer[rrq_len], "octet");
    rrq_len += strlen("octet") + 1;  // +1 for null character
